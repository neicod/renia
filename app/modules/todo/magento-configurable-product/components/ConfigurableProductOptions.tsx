...
export const ConfigurableProductOptions: React.FC<Props> = ({
  ...
  const configurableOptions = Array.isArray((product as any)?.configurableOptions)
    ? (product as any).configurableOptions
    : [];

  const sortedOptions = React.useMemo(
    () => [...configurableOptions].sort((a: any, b: any) => Number(a?.position ?? 0) - Number(b?.position ?? 0)),
    [configurableOptions]
  );
