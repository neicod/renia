...
export const CategoryProductList: React.FC<Props> = ({ meta, initialListing: initialListingProp }) => {
  const { category, uid: categoryUid, uidStatus, path: categoryPath } = useCategoryContext({ meta });
  const categoryLabel = typeof (category as any)?.label === 'string' ? (category as any).label : undefined;

  // Merge initialListing from props (explicit) or meta (routeMeta from SSR or /api/page-context).
  // Note: AppRoot delays rendering catch-all routes until /api/page-context resolves, so meta should not be stale here.
  const initialListing = React.useMemo(
    () =>
      initialListingProp ??
      ((meta as any)?.categoryProductListing ?? null),
  ...
  } = useCategoryProductList({ categoryUid, categoryPath, initialListing });
