...
export default function customerInterceptor(api: any = {}) {
  ...
  api.layout.at('control-menu').add('renia-magento-customer/components/CustomerStatusControl', 'customer-status-control', {
    sortOrder: { before: '-' }
  });

  api.layout.at('global-overlay').add('renia-magento-customer/components/CustomerBootstrap', 'customer-bootstrap', {
    sortOrder: { before: '-' }
  });
}
