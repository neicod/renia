...
export const getGlobalTtlCache = <V>(name: string, options: Omit<TtlCacheOptions, 'name'>): TtlCache<V> => {
  const key = `__RENIA_TTL_CACHE__:${name}`;
  const globalAny = globalThis as any;
  const existing = globalAny[key] as TtlCache<V> | undefined;
  if (existing) return existing;
  const created = new TtlCache<V>({ ...options, name });
  globalAny[key] = created;
  return created;
};

export const listGlobalTtlCaches = (): Array<ReturnType<TtlCache<any>['stats']> & { key: string }> => {
  const globalAny = globalThis as any;
  const prefix = '__RENIA_TTL_CACHE__:';
  const keys = Object.keys(globalAny).filter((k) => k.startsWith(prefix));
  const out = keys
    .map((key) => {
      const cache = globalAny[key] as any;
      const stats = cache && typeof cache.stats === 'function' ? cache.stats() : {};
      return { key, ...(stats as any) };
    })
