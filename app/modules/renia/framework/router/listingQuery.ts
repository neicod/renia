...
const DEFAULTS_FALLBACK: Required<ListingQueryDefaults> = {
  page: 1,
  pageSize: 0,
  sort: 'relevance'
};
...
export const readListingQueryState = (
  search: string | null | undefined,
  defaults: ListingQueryDefaults = {}
): ListingQueryState => {
...
export const applyListingQuery = (
  search: string | null | undefined,
  next: ApplyListingQueryArgs,
  defaults: ListingQueryDefaults = {}
): string => {
  const d = { ...DEFAULTS_FALLBACK, ...(defaults ?? {}) };
  const normalizedSearch = dedupeSearch(search);
 ...
 */
export const normalizeSearchTermKey = (search: string | null | undefined): string => {
  const normalizedSearch = dedupeSearch(search);
  const params = new URLSearchParams(normalizedSearch.startsWith('?') ? normalizedSearch.slice(1) : normalizedSearch);
