...
const getEntryComponentKey = (entry) => {
    if (typeof entry.componentPath === 'string' && entry.componentPath)
        return entry.componentPath;
...
export const validateLayoutArtifacts = ({ regions, extensions, isComponentRegistered, knownRegions = defaultKnownRegions }) => {
    const issues = [];
        ...
        for (const entry of entries) {
            ...
            if (typeof entry.component === 'function') {
                issues.push({
                    ...
                    message: `Region "${regionName}" contains a direct React component reference; register it and use componentPath string instead.`,
