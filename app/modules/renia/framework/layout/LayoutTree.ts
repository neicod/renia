...
export class LayoutTreeBuilder {
  ...
  get(path: string): LayoutNodeAPI {
    throw new Error(
      `[Layout] LayoutTreeBuilder.get(...) is disabled. Use LayoutTreeBuilder.at(...) instead. (path="${path}")`
    );
  }
...
export class LayoutNodeAPI {
  ...
  get(id: string): LayoutNodeAPI {
    throw new Error(
      `[Layout] layout.get(...) is disabled. Use layout.at(...) instead. Called with: ${this.node.path}.get('${id}')`
    );
  }
