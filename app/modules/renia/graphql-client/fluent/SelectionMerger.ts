...
const argsEqual = (a: ArgValue | undefined, b: ArgValue | undefined): boolean => {
  if (a === b) return true;
...
export class SelectionMerger {
  ...
  private mergeNode(target: SelectionNode[], incoming: SelectionNode) {
    if (incoming.fragment) {
      const exists = target.some((n) => n.fragment === incoming.fragment);
      if (!exists) target.push(incoming);
      return;
    }
