...
const responseKeyOf = (node: SelectionNode): string => node.alias ?? node.name;

export class FieldBuilder {
  constructor(
    private readonly getNode: () => SelectionNode,
  ...
  getSelection(): SelectionBuilder {
    ...
    return new SelectionBuilder(() => (node.children ??= []), this.pathLabel);
  }
...
export class SelectionBuilder {
  private readonly parser = new SnippetParser();
  private readonly merger: SelectionMerger;

  constructor(
    private readonly getSelectionNodes: () => SelectionNode[],
    private readonly pathLabel: string
  ) {
  ...
  remove(fieldOrResponseKey: string): this {
    ...
    const next = nodes.filter((n) => {
      if (isSpread) return !(n.fragment === needle);
  ...
  field(name: string, opts?: FieldOptions): FieldBuilder {
    const n = name.trim();
    if (!n) throw new Error('Field name cannot be empty');
    const responseKey = opts?.alias ?? n;
    const nodes = this.selection();
    const existing = nodes.find((x) => !x.fragment && !x.inline && responseKeyOf(x) === responseKey);
    if (!existing) {
      const created: SelectionNode = { name: n, alias: opts?.alias, args: opts?.args, children: [] };
      nodes.push(created);
  ...
  at(path?: string): SelectionBuilder {
    ...
    for (const segment of segments) {
      ...
      current = new SelectionBuilder(
        () => (node.children ??= []),
        current.joinPath([responseKeyOf(node)])
      );
    }
    return current;
  }

  private joinPath(next: string[]): string {
