...
import { isConfigurableProduct, ConfigurableProductOptions, useConfigurableSelection } from '../index';
 ...
 * For simple products: returns null (allows default AddToCartButton from magento-cart)
 */
export const ConfigurableProductListingWrapper: React.FC<Props> = ({ product }) => {
  // Hooks MUST be called in same order on every render - call before any conditionals
  const [isClient, setIsClient] = React.useState(false);
  const [adding, setAdding] = React.useState(false);
  const toast = useToast();
  const manager = useCartManager();
  const { t } = useI18n();
  const { currentVariant } = useConfigurableSelection(product);
  ...
  const handleAdd = React.useCallback(async () => {
    ...
    setAdding(true);
    try {
      await manager.addProduct({ sku: currentVariant.product.sku, quantity: 1 });
      toast({
        tone: 'success',
  ...
  }, [currentVariant, product, toast, manager, t]);

  // Return null on SSR (isClient === false) to avoid hydration mismatch
  if (!isClient || !isConfigurable) {
    return null;
  }
