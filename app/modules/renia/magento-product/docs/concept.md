...
- **Subsloty:** `ProductTile` publikuje `product-listing-actions`, a `ProductDetails` `product-view-actions`, co pozwala innym modułom (np. koszykowi) wstrzykiwać swoje akcje przez hierarchiczny API (`api.layout.get('product-listing-actions').add(...)`).
