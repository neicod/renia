...
| `renia-magento-cart-sidebar` | Wysuwany sidebar koszyka na wszystkich stronach poza `/cart`. | `components/CartSidebar.tsx`, `components/CartLinkSidebar.tsx`, `services/cartSidebarStore.ts`, `interceptors/default.ts`. | Interceptor podmienia `cart-link` w slocie `control-menu`, a dane pobiera z `renia-magento-cart`/`renia-module-cart`; brak SSR, wszystko po stronie klienta. |
